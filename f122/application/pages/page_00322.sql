...
'      $s(''P322_DIR_AUX_SEQ_ID'', cb.value,false);',
'  } else {',
...
,p_last_updated_by=>'VROTELA'
,p_last_upd_yyyymmddhh24miss=>'20240227162158'
);
...
,p_button_sequence=>10
,p_button_plug_id=>wwv_flow_imp.id(80147848680643726)
...
,p_error_message=>'pARA aNULAR DE'
...
'			APEX_DEBUG.ERROR(SQLERRM);RAISE ALGUN_ERROR;',
'	END;',
'		IF NVL( VAFECTASTOCK, ''N'' ) = ''S''  THEN VCANT_PEND := 0;',
'			BEGIN ',
'				IF NVL(:P322_RETIRA_DE,''N'') NOT IN (''CDE'',''SLO'') AND NVL(:P322_COL_IND_OFERTA,''N'') <> ''S'' THEN',
'				BEGIN',
...
'				EXCEPTION',
'					WHEN OTHERS THEN ',
'						APEX_DEBUG.ERROR(SQLERRM);VCANTIDAD := 0;',
'				END;',
'				END IF;',
...
'	:P322_FEC_CONFIRMACION := SYSDATE;',
'	:P322_HORA_CONFIRMACION := TO_CHAR(SYSDATE, ''HH24:MI:SS'');',
...
wwv_flow_imp_page.create_page_da_action(
 p_id=>wwv_flow_imp.id(347921130617606735)
...
'EXCEPTION',
'	WHEN OTHERS THEN',
'		APEX_DEBUG.ERROR(''Error en el guardado de la cabecera: ''||SQLERRM);',
'END;',
''))
...
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'BEGIN',
'    APEX_DEBUG.ERROR(''ANTES DE GUARDAR_VTREPDDET'');',
'    INV.VTREPDET.GUARDAR_VTREPDET;',
'EXCEPTION',
'    WHEN OTHERS THEN',
'        APEX_DEBUG.ERROR(SQLERRM);',
